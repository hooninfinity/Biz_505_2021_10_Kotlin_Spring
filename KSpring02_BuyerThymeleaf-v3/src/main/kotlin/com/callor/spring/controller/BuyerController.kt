...
// @RequestParam("userid") 가 없어도 됨, 대신 userid:String 에선 userid 변수이름이 틀리면 안됨
     ...
     * Constroller 에서 Model 객체를 담고
     * form 화면을 rendering 하도록 하면
     *
     * 보통은 form 의 value 속성에 일일히
     * 데이터를 추가하여 작성을 한다
     *
     * ModelAttr 을 사용하면
     ...
     * 각 input box 에서는 field 속성으로 해당 멤버변수(요소,속성)을
     * 지정해 주면
...
//        val insertBuyer = ConfigData.BUYER_LIST[0]
    ...
    /**
     * update 를 실행할때
     *      localhost:8080/buyer/update/userid 값 으로 URL 이 구성되어 있고
     *      update 화면에서 저장을 누르면
     *      원래 요청했던 주소가 action 이 되어 요청되므로
     *      여기에서는 userid 필요없지만
     *      임의로 PathVariable 로 설정해 주어야 한다
     */
    ...
    fun update(redirectAttributes: RedirectAttributes, buyer: Buyer, @PathVariable("userid") userid: String) : String{
         ...
         * redirect 를 실행할때 model 에 담긴 변수를
